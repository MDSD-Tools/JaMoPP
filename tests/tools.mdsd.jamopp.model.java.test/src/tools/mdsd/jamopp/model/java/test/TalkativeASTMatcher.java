...
/**
 * An extension of the JDT ASTMatcher that prints information whenever the matching of two ASTs
 * fails. In addition some normalizations are applied to number literals to removed redundant
 * characters that can cause comparison failures even though the numbers are the same.
 */
public class TalkativeASTMatcher extends ASTMatcher {
    ...
    private static String normalizeNumberToken(String token) {
        if (token.contains("\\u")) {
            ...
            for (int index = 0; index < token.length(); index++) {
                final char currentChar = token.charAt(index);
                if (currentChar == '\\') {
                    final int codePoint = Integer.parseInt(token.substring(index + 2, index + 6), 16);
        ...
        if (token.startsWith("-0x")) {
            return token.substring(1) + "-" + normalizeNumberToken(token);
        }
    ...
    public boolean match(NumberLiteral node, Object other) {
        ...
        final String nToken = node.getToken();
    ...
    public boolean match(PrefixExpression node, Object other) {
        ...
        if (other instanceof NumberLiteral) {
            final String nToken = node.toString();
            final String oToken = ((NumberLiteral) other).getToken();
            return setDiff(node, other, numberMatch(nToken, oToken));
        }
        return setDiff(node, other, super.match(node, other));
