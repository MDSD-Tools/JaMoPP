...
public class JamoppJavaParserFactoryImpl implements JamoppJavaParserFactory {
	...
	public ASTParser getJavaParser(String version) {
		...
		int astParserLevel;
		final var stripedVersion = String.valueOf(version).strip();
		...
		default:
			...
			this.logger.warn(String.format("No version was found for %s, so the last supported version is used.",
					stripedVersion));
			break;
		}
		final var parser = ASTParser.newParser(astParserLevel);
